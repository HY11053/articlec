        ...
        <div class="col-md-3 col-sm-6 col-xs-12">
            <div class="info-box">
                ...
                <div class="info-box-content">
                    <span class="info-box-text">内容模型总数</span>
                    <span class="info-box-number">{{\App\AdminModel\ContentSource::count()}}</span>
                </div>
                <!-- /.info-box-content -->
            </div>
            <!-- /.info-box -->
            ...
            <div class="box">
                <div class="box-header with-border">
                    ...
                    <span style="margin-left: 5px; display: inline-block;">
                        ...
                        今日内容模型添加数量：{{\App\AdminModel\ContentSource::where('created_at','>',Carbon\Carbon::today())->where('created_at','<',\Carbon\Carbon::tomorrow())->count()}}
                    </span>
                ...
                <div class="box-body">
                    <div class="row">
                        <div class="col-md-8">
                            <p class="text-center">
                                <strong>截止当前: {{date('Y-m-d H:i:S',time())}}内容分类数据总览</strong>
                            </p>
        ...
        <div class="col-md-12">
            <div class="box">
                ...
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        <ul class="todo-list">
                            @foreach($latestcontents as $index=>$latestcontent)
                                <li>
                                    <!-- drag handle -->
                                    <span class="handle">
                        ...
                        <i class="fa fa-ellipsis-v"></i>
                                    ...
                                    <!-- todo text -->
                                    <span class="text">{{mb_substr($latestcontent->content,0,50,'utf-8')}}...</span>
                                    <!-- Emphasis label -->
                                    <small class="label {{$labelStyle[$index]}} pull-right" style="font-weight: normal;"><i class="fa fa-clock-o"></i> {{Carbon\Carbon::parse($latestcontent->created_at)->diffForHumans()}}—{{$latestcontent->write}}</small>
                ...
                datasets: [

                    {
                        ...
                        data: [
                            @foreach($articletypes as $type)
                                @if(!$loop->last)
                                    {{\App\AdminModel\ContentSource::where('content_type',$type->id)->count()}},
                                @else
                                     {{\App\AdminModel\ContentSource::where('content_type',$type->id)->count()}}
                                @endif
                            @endforeach
                    ...
                    },
                    {
                        ...
                        data: [
                            @foreach($articletypes as $type)
                            @if(!$loop->last)
                            {{\App\AdminModel\ContentSource::where('content_type',$type->id)->where('created_at','>',\Carbon\Carbon::today())->where('created_at','<',\Carbon\Carbon::tomorrow())->count()}},
                            @else
                            {{\App\AdminModel\ContentSource::where('content_type',$type->id)->where('created_at','>',\Carbon\Carbon::today())->where('created_at','<',\Carbon\Carbon::tomorrow())->count()}}
                            @endif
                            @endforeach
